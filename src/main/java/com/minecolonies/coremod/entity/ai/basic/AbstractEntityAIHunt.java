...
import com.minecolonies.api.entity.ai.citizen.hunter.HunterGearBuilder;
...
public abstract class AbstractEntityAIHunt <J extends JobHunter, B extends BuildingHunter> extends AbstractEntityAIInteract<JobHunter, BuildingHunter>
{
    ...
    /**
     * List of items that are required by the hunter based on building level and hunter level.  This array holds a pointer to the building level and then pointer to HunterGear
     */
    public final List<List<HunterGear>> itemsNeeded = new ArrayList<>();
    ...
    /**
     * The value of the speed which the hunter will move.
     */
     ...
     */
    public AbstractEntityAIHunt(@NotNull final J job)
    {
        super(job);
        ...
        itemsNeeded.add(HunterGearBuilder.buildGearForLevel(ARMOR_LEVEL_IRON, ARMOR_LEVEL_MAX, LEATHER_LEVEL_RANGE, DIA_BUILDING_LEVEL_RANGE));
        itemsNeeded.add(HunterGearBuilder.buildGearForLevel(ARMOR_LEVEL_CHAIN, ARMOR_LEVEL_DIAMOND, LEATHER_LEVEL_RANGE, DIA_BUILDING_LEVEL_RANGE));
        itemsNeeded.add(HunterGearBuilder.buildGearForLevel(ARMOR_LEVEL_LEATHER, ARMOR_LEVEL_IRON, LEATHER_LEVEL_RANGE, IRON_BUILDING_LEVEL_RANGE));
        itemsNeeded.add(HunterGearBuilder.buildGearForLevel(ARMOR_LEVEL_LEATHER, ARMOR_LEVEL_CHAIN, LEATHER_LEVEL_RANGE, CHAIN_BUILDING_LEVEL_RANGE));
        itemsNeeded.add(HunterGearBuilder.buildGearForLevel(ARMOR_LEVEL_LEATHER, ARMOR_LEVEL_GOLD, LEATHER_LEVEL_RANGE, GOLD_BUILDING_LEVEL_RANGE));
    }

    protected abstract int getAttackRange();

    /**
     * Redirects the hunter to their building.
     *
    ...
    /**
     * Prepares the hunter. Fills his required armor and tool lists and transfer from building chest if required.
     *
     * @return The next {@link IAIState}.
     */
    ...
    /**
     * Task to do when at the own building, as hunter only go there on requests and on dump
     */
    protected void atBuildingActions()
    {
        final BuildingHunter building = getOwnBuilding();
        for (final List<HunterGear> itemList : itemsNeeded)
        {
            for (final HunterGear item : itemList)
            {
    ...
    /**
     * This gets the attack speed for the hunter with adjustment for hunter level. Capped at 2
     *
     * @return movement speed for hunter
     */
